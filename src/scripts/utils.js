...
export function sort_subvers(subvers) {
    return subvers.sort((a, b) => {
        if (a.toLowerCase().endsWith("-snapshot")) {
            let suffixless_compare = -natural_compare(a.slice(0, -"-snapshot".length), b);
            if (suffixless_compare == 0) {
                return 1;
            } else {
                return suffixless_compare;
            }
        } else if (b.toLowerCase().endsWith("-snapshot")) {
            let suffixless_compare = -natural_compare(a, b.slice(0, -"-snapshot".length));
            if (suffixless_compare == 0) {
                return -1;
            } else {
                return suffixless_compare;
